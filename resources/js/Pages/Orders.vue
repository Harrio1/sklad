                            ...
                            <div class="w-3/4">
                                ...
                                <div :class="{'grid grid-cols-1': displayMode === 'single', 'grid grid-cols-2 gap-4': displayMode === 'double', 'grid grid-cols-3 gap-2': displayMode === 'compact'}">
                                    <div v-for="order in filteredOrders" :key="order.id" 
                                         ...
                                         class="cursor-pointer p-4 border-b border-gray-200 dark:border-gray-700">
                                        ...
                                        <div v-if="order.showDetails" class="mt-2">
                                            <ul>
                                                <li v-for="item in JSON.parse(order.products)" :key="item.id" class="flex justify-between items-center mb-2">
                                                    <span class="font-bold text-gray-800 dark:text-gray-200">{{ item.name }} - {{ item.quantity }}</span>
                                                    <span class="text-gray-800 dark:text-gray-200">{{ (item.quantity * item.total_price).toFixed(2) }} ₽</span>
                                                </li>
                                            </ul>
                                            <hr class="my-2">
                                            <div class="flex justify-between items-center mt-2">
                                                <span class="font-semibold text-gray-800 dark:text-gray-200">Итого:</span>
                                                <span class="font-bold text-gray-800 dark:text-gray-200">{{ JSON.parse(order.products).reduce((total, item) => total + item.quantity * item.total_price, 0).toFixed(2) }} ₽</span>
                                            </div>
                                            <div class="mt-2">
                    ...
                    <div v-else>
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 mb-6">
                            ...
                            <div v-for="product in products" :key="product.id" class="flex justify-between items-center mb-4 p-4 border-b border-gray-200 dark:border-gray-700">
                                <div class="text-gray-700 dark:text-gray-300 font-semibold">{{ product.name }} - {{ product.total_price.toFixed(2) }} ₽</div>
                                <div class="flex items-center">
                                    ...
                                    <span class="mx-3 text-gray-700 dark:text-gray-300">{{ product.quantity }}</span>
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                            ...
                            <div v-for="item in cart" :key="item.id" 
                                 class="flex justify-between items-center mb-2 p-2 border-b border-gray-200 dark:border-gray-700">
                                <div class="text-gray-700 dark:text-gray-300">{{ item.name }} - {{ item.quantity }} x {{ item.total_price.toFixed(2) }} ₽</div>
                                <div class="text-gray-700 dark:text-gray-300">{{ (item.quantity * item.total_price).toFixed(2) }} ₽</div>
                            </div>
                            <div class="flex justify-between items-center mt-4">
                                <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">Итого:</div>
                                <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ totalPrice.toFixed(2) }} ₽</div>
                            </div>
